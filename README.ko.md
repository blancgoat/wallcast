...
| 사용자 크기 / 맞춤 | 예: `2732x2048` · **Fit to screen**, Actual pixels (압축이 아니라 자르기) |
...
**표시 비율**은 캡처된 프레임에서 영역을 **잘라냅니다.** 캡처보드는 4:3 소스를 자기 16:9 프레임에
필러박스로 넣기 때문에 검은 여백이 그림의 일부로 들어옵니다. 그걸 잘라내는 게 이 옵션의 목적입니다.
찌그러뜨려 맞추는 일은 없습니다 — 왜곡되는 선택지는 `Stretch to screen` 하나뿐입니다.
`Input resolution`은 아무것도 자르지 않고 들어온 그대로 보여줍니다.

`Custom size`는 아이패드의 `2732x2048` 같은 임의의 크기를 받고, 맞춤 방식이 그걸 어떻게 읽을지
정합니다:

- `Fit to screen`은 **모양만** 씁니다. 그 모양의 가장 큰 영역을 프레임에서 잘라낸 뒤 화면이 허용하는
  만큼 크게 그립니다. 4:3 소스를 3840x2160으로 캡처한 경우 정확히 `2880x2160`, x=480이 되는데
  여백이 끝나는 바로 그 지점입니다.
- `Actual pixels`는 **픽셀 수 그대로** 씁니다. 프레임 가운데에서 딱 그만큼 잘라내고 소스 1픽셀을
  화면 1픽셀로 그립니다. 어디에서도 리스케일하지 않습니다.

...
동영상은 원본 모양을 유지하며, 이음매 없이 반복됩니다. 플레이어를 멈췄다 다시 시작하는 대신 입력 자체를 반복시키므로, 한 바퀴 돌 때마다 0.2초쯤 바탕화면이 드러나던 현상이 없어졌습니다. 기본 음소거이며 해제할 수 있습니다. 두 입력 모두 같은 방식으로 화면에 배치할 수 있습니다. [배치](#배치) 참고. 캡처는 영상만 출력합니다. 캡처 버퍼 값은 DirectShow 큐 용량을 정하는 기준이며 정확한 지연 시간이 아닙니다. 큐 용량은 장치가 내보내는 형식의 프레임 크기로 계산하므로, 같은 값이면 해상도가 올라가도 담기는 프레임 수는 같습니다. 화면 출력은 가장 최근 프레임을 사용하며, 프레임이 도착할 때마다 바로 표시합니다.

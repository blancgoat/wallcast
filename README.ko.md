...
| 사용자 크기 / 맞춤 | 예: `2732x2048` · **Crop, fit to screen**, Stretch frame to this shape |
...
- `Crop, fit to screen` — **모양만** 씁니다. 그 모양의 가장 큰 영역을 프레임에서 잘라내 화면이
  허용하는 만큼 크게 그립니다. 필러박스를 넣는 캡처보드용입니다. 4:3 소스를 3840x2160으로 캡처하면
  정확히 `2880x2160`, x=480 — 여백이 끝나는 바로 그 지점입니다.
...
고정 비율이 필요해지는 날이 오면 그건 자르기가 아니라 늘리기로 필요할 것이기 때문입니다. 모양이 아니라 정확한 픽셀 수로 잘라내는 기능도 아직 넣지 않았습니다.

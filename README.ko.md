...
**표시 비율**은 캡처된 프레임을 바탕화면에 올리기 전에 어떻게 처리할지 정합니다.
`Input resolution`은 그대로 둡니다. `Stretch to screen`은 모니터를 가득 채우며 그러느라 왜곡됩니다.
`Output resolution`이 OBS처럼 동작하는 쪽입니다. `2753x2064` 같은 크기를 주면 **캡처 해상도가
...
- `Centre at 1:1, padding the gap` — 전혀 확대·축소하지 않습니다. 출력이 허용하는 만큼만 프레임
  가운데에서 가져와 소스 1픽셀을 화면 1픽셀로 그립니다. 캡처에 없는 픽셀을 만들어내지 않으므로
  1920x1080 캡처는 출력을 더 크게 잡아도 1920x1080에 머뭅니다.
...
없으면 그리드가 비활성으로 바뀝니다 — 16:9 캡처를 16:9 모니터에 통째로 올리면 이미 전부 덮고 있고, 화면에 늘린 경우도 마찬가지입니다. 자르기 자체는
언제나 프레임 가운데에서 가져옵니다. 필러박스는 여백을 양옆에 두기 때문입니다.

`Input resolution`과 `Stretch to screen`은 캡처 모양이 모니터와 같을 때 — 대개의 경우 — 결과가
...
자르기는 캡처 엔진에서 일어나므로 여백이 애초에 파이프를 타고 오지 않습니다.
...
- `CaptureOptions.cs`: 픽셀 형식·해상도·FPS·YUV 변환, 그리고 캡처보드의 검은 여백을 떼어내는 자르기.
...
자동 검증은 `dotnet run --project SmokeTests -c Release -r win-x64 --self-contained true`로 실행합니다. VLC 로딩, 장치 검색, 잘못된 입력, 출력 해상도와 매핑 네 가지, 화면 위치, 실제 FFmpeg의 NV12 빨강/파랑 변환, Limited/Full, Rec.709/601 차이, 재생 정리를 검사합니다.

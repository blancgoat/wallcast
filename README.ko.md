...
| 사용자 크기 / 맞춤 | 예: `2732x2048` · **Crop, fit to screen**, Crop actual pixels, Stretch frame to this shape |
| 자르기 기준점 | 3x3 그리드, **Center** |
...
- `Crop, actual pixels` — **픽셀 수 그대로** 씁니다. 딱 그만큼 잘라내고 소스 1픽셀을 화면 1픽셀로
  그립니다. 어디에서도 리스케일하지 않습니다.
- `Stretch frame to this shape` — 아무것도 자르지 않습니다. 프레임 전체를 그 모양으로 만듭니다.
  필러박스 대신 소스를 프레임에 눌러 담는 구형 캡처보드용입니다. 뗄 여백이 없고 비율만 되돌리면
  되는 경우입니다.

**자르기 기준점**은 3x3 그리드로, 캔버스 크기 조정의 기준점처럼 읽습니다. 프레임의 어느 부분을
남길지 고릅니다. 두 자르기 모드에 적용되고, 자르는 게 없는 늘리기에서는 무시됩니다. 그림 자체는
언제나 모니터 가운데에 놓입니다.

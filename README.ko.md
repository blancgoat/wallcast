...
| 색 범위 | **Limited**, Full |
| 표시 비율 | **Input resolution**, Stretch to screen, Custom size |
| 사용자 크기 / 맞춤 | 예: `2732x2048` · **Crop to this shape**, Crop this many pixels, Stretch frame to this shape |
...
`Input resolution`은 그대로 둡니다. `Stretch to screen`은 모니터를 가득 채우며, 왜곡되는 선택지는
이것 하나뿐입니다. `Custom size`는 아이패드의 `2732x2048` 같은 임의의 크기를 받고, **맞춤 방식**이
그걸 어떻게 읽을지 정합니다:

- `Crop to this shape` — 크기를 **비율로만** 읽습니다. 그래서 `2732x2048`과 `1024x768`이 완전히
  같게 동작합니다. 그 모양의 가장 큰 영역을 프레임에서 잘라내 화면이 허용하는 만큼 크게 그립니다.
  필러박스를 넣는 캡처보드용입니다. 4:3 소스를 3840x2160으로 캡처하면 정확히 `2880x2160`, x=480 —
  여백이 끝나는 바로 그 지점입니다.
- `Crop this many pixels` — **숫자 그대로** 읽습니다. `2668x1500`이면 프레임 가운데에서 2668×1500
  픽셀을 잘라내 소스 1픽셀을 화면 1픽셀로 그립니다. 숫자를 알고 있을 때, 그리고 비율로는 뜻이
  전달되지 않을 때 씁니다 — `2668x1500`은 16:9라서 모양으로 읽으면 아무것도 바뀌지 않습니다.
- `Stretch frame to this shape` — 아무것도 자르지 않습니다. 프레임 전체를 그 모양으로 만듭니다.
  필러박스 대신 소스를 프레임에 눌러 담는 구형 캡처보드용입니다. 뗄 여백이 없고 비율만 되돌리면
  되는 경우입니다.

설정 아래 줄이 지금 숫자가 무엇을 하는지 — 프레임에서 무엇을 잘라내 얼마나 크게 어디에 그리는지 —
그대로 보여주므로 어느 쪽으로 읽히는지 짐작할 필요가 없습니다.

**화면 위치**는 3x3 그리드로, 캔버스 크기 조정의 기준점처럼 읽습니다. 그림이 모니터의 어디에 놓일지
정합니다. 남는 자리가 있어야 의미가 있으므로, 없으면 그리드가 비활성으로 바뀝니다 — 16:9 캡처를
